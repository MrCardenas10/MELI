...
const itemsData = {
    ...
    items: [
        ...
        {
            id: "testid2",
            title: "Apple iPhone 14 Plus 256gb Con Pantalla Táctil Color Amarillo Busca Que Todo Se Vea Más Nítido Y Real",
            price: {
                currency: "USD",
                amount: 2203,
                decimals: 0
        ...
        <div className={"w-full h-5/6 bg-white mt-8 rounded-sm p-2"}>
            {itemsData.items.map((product, idx) => (
                    <Fragment key={product.id}>
                        <div className={"flex w-full cursor-pointer"} onClick={() => {
                            onClickProductDetail(product.id)
                        }}>
                            <div className={"w-[180px] h-[180px] mx-4 bg-red-500 rounded-[4px]"}>
                                <img src="/src/assets/images/xiaomi.webp" alt="xiaomi"/>
                            </div>
                            <div className={"flex flex-col w-6/12 h-40 justify-center"}>
                                <span className={"font-normal text-[#333333] text-2xl mb-8"}>$ {product.price.amount.toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, ".")}</span>
                                <span className={"font-normal text-[#666666] text-lg"}>{product.title}</span>
                            </div>
                            <div className={"flex w-6/12 justify-center h-32 items-center"}>
                                <span className={"text-[#999999] text-sm"}>{product.condition}</span>
                            </div>
                        </div>
